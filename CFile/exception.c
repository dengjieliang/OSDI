...
{
    async_uart_puts("\n[DEFAULT EXCEPTION]\n");
    async_uart_puts("  ESR_EL1 = 0x"); 
    async_uart_send_unsigned_long_integer(esr);  
    async_uart_puts("\n");
    async_uart_puts("  ELR_EL1 = 0x"); 
    async_uart_send_unsigned_long_integer(elr);  
    async_uart_puts("\n");
    ...
    async_uart_puts("  FAR_EL1 = 0x"); 
    async_uart_send_unsigned_long_integer(far);
    async_uart_puts("\n");
...
void el0_sync_handler_c(unsigned long esr, unsigned long elr, unsigned long spsr, unsigned long *ctx)
{
    ...
    async_uart_send_unsigned_long_integer(spsr); 
    async_uart_puts("\n");
    async_uart_puts("  EC      = 0x"); 
    async_uart_send_unsigned_long_integer(exception_class);   
    async_uart_puts("\n");
    ...
    if (exception_class == 0x15) // SVC from AArch64
    {
        ...
        async_uart_puts("  ISS     = 0x"); 
        async_uart_send_unsigned_long_integer(imm16);   
        async_uart_puts("\n");
        return;
    }
